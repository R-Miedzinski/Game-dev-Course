#pragma once
#include <glm/glm.hpp>
#include <glm/ext.hpp>
#include <SFML/Window.hpp>

class Camera {
public:
	Camera(const glm::vec3& position, const glm::vec3& front, float yaw, float pitch, sf::Vector2<uint> windowSize);

	glm::mat4 View() const { return m_lookAt; }
	glm::mat4 Projection() const { return m_projection; }
	glm::vec3 Postion() const { return m_position; }
	glm::vec3 Direction() const { return m_front; }

	void Rotate(const sf::Vector2i& mouseDelta);
	void MoveForward(float velocity);
	void MoveBackward(float velocity);
	void MoveLeft(float velocity);
	void MoveRight(float velocity);
	void MoveUp(float velocity);
	void MoveDown(float velocity);
	void ResizeWindow(sf::Vector2<uint> windowSize);

private:
	void RecreateLootAt();

	glm::mat4 m_projection;
	glm::mat4 m_lookAt;

	glm::vec3 m_position;
	glm::vec3 m_up;
	glm::vec3 m_front;
	glm::vec3 m_right;
	float m_yaw;
	float m_pitch;

	static glm::vec3 s_worldUp;
};